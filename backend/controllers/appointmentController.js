...
// this function detects the frontend URL from env, request, or host (fallback)
...
// this function safely finds and returns the Clerk user ID from the request (from multiple possible places)
...
export const getAppointments = async (req, res) => {
  try {
    ...
    console.error("Get Apointment error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};

// to getApointment by Patient
export const getAppointmentsByPatient = async (req, res) => {
  try {
    ...
    if (!resolvedCreatedBy) {
      return res
        .status(401)
        .json({ success: false, message: "Authenticated required" });
    }
