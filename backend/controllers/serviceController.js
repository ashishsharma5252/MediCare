...
// This function takes any input (array, JSON string, or comma string) and converts it into a proper array
...
// This function groups appointment slots by date and converts them into a clean object format.
...
// cleans any price value and converts it into a number
const sanitizePrice = (v) =>
  Number(String(v ?? "0").replace(/[^\d.-]/g, "")) || 0;

// Converts availability into true (available) or false (not available).
...
export async function updateService(req, res) {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, data: updated, message: "Service updated successfully" });
  } catch (err) {
